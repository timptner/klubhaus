...
class EventListView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        ...
        context["my_events"] = self.request.user.volunteer_set.values_list(
            "event__pk", flat=True
        )
        return context
...
class EventDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if event.state == Event.PREPARED:
            ...
            color = "is-warning"
            msg = "Du bist für diese Veranstaltung bereits registriert."
        elif not self.request.user.phone:
            url = reverse_lazy("accounts:profile")
            color = "is-danger"
            msg = (
                "Bitte ergänze deine Mobilnummer in deinem "
        ...
        context["is_volunteer"] = event.volunteer_set.filter(
...
class VolunteerCreateView(
    LoginRequiredMixin, UserPassesTestMixin, SuccessMessageMixin, CreateView
):
    model = Volunteer
    form_class = VolunteerForm
    success_message = "Du hast dich erfolgreich angemeldet."
...
class VolunteerContactView(PermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        if errors:
            ...
            messages.success(
                self.request,
                f"Es wurden alle freiwilligen Helfer erfolgreich per E-Mails kontaktiert.",
            )
    ...
    def get_success_url(self):
        return reverse_lazy(
            "volunteers:volunteer_list", kwargs={"pk": self.kwargs["pk"]}
        )
...
def volunteer_export(request, pk):
    ...
    for volunteer in event.volunteer_set.order_by(
